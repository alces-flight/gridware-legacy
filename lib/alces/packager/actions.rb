...
EOF
      ...
      def modules
        ...
        module_loading = modules.map do |m|
          "eval `#{ENV.fetch('MODULESROOT','/opt/modules')}/bin/modulecmd sh load #{m} || echo false`"
        end.join("\n")
        <<-BASH
eval `#{ENV.fetch('MODULESROOT','/opt/modules')}/bin/modulecmd sh purge || echo false` || true
#{module_loading}
